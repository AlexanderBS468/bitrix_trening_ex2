...
namespace Techdir\PhpInterface\Agent;
...
class Controller
{
	 ...
	 * */
	public static function getRegisteredAgents($baseClassName)
	{
		$registeredList = array();
		$namespaceLower = strtolower('\\' . __NAMESPACE__);
		...
		while ($agentRow = $query->fetch())
		{
			...
			$agentClassName = $agentCallParts[0];

			if (
				!empty($agentClassName)
				&& (
